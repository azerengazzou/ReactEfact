...
const show = () => {
  ...
  console.log(userRef);
  setToken(token);
   axios.get(`https://localhost:7287/gateway/Ticket/${id}`, {
  })
    .then(response => {
      ...
      console.log("message : " + data.discussions[0].messages);
      axios.get(`https://localhost:7287/gateway/Auth/${data.userRef}`, {
      })
        .then(response => {
...
export default DiscussionDoctor;